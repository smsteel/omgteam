...
//        }
//        return self::$object;
        return new self($GLOBALS['config']['tpl_folder']);
    }
    ...
    private function yes_print($id)
    {
        ...
        for ($i=0; $i<$num; $i++)
        {
			...
			if (($i+1) == $num) return 0; else fseek($this->file, $pols);
        }
    }
    ...
    private function string_rps($str, $id = 0)
    {
        ...
        for($i=0; $i<strlen($str); $i++)
		{
			if ($str[$i] == "{")
            {
				...
				if (isset($this->value[$index]) == false) $this->value[$index] = '';

                if (gettype($this->value[$index]) == 'array')
                {
                    if ($num < count($this->value[$index])) {$num = count($this->value[$index]);}

					if (isset($this->value[$index][$id]) == false) $this->value[$index][$id] = '';
	...
	private function name($text)
	{
		...
		for ($i=7; $i<strlen($text); $i++)
		{
			...
			{
				$i++;
				if ($text[$i] != " ") $return_text .= $text[$i];  else break;
			}
			
			if ($return_text != "") break;
		}
    ...
    private function hendler($i = 0, $old_id = '')
    {
        ...
        while(!feof($this->file))
        {
            ...
            if (substr($str, 0, 11) == "<!-- BEGIN ")
            {
                ...
                if (isset($this->show[$id]) == false) $this->show[$id] = $this->defolt;
